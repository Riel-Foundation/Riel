...
pub fn add_files(subcommands: Vec<String>, options: Vec<String>) -> bool {
    ...
    let should_add_all: bool =
        options.contains(&("-all".to_string())) || options.contains(&("-A".to_string()));
            ...
            if should_add_all {
                ...
                if ignored.exists {
                    ...
                    let ignore_list: Vec<String> =
                        ignored.files.iter().map(|x| x.to_string()).collect();
                    let fixed_ignore_list: Vec<String> =
                        ignore_list.iter().map(|x| format!("./{}", x)).collect();
...
fn copy_to_area(items: Vec<String>) -> bool {
    for item in items {
        ...
        if fs::metadata(format!("./{}/", &item)).is_ok() {
        ...
        } else if fs::metadata(&item).is_ok() {
            // file
            fs::copy(&item, format!(".riel/area/{}", &item)).expect("Failed to copy item.");
            println!("Prepared {}.", item);
        } else {
            println!(
                "{} does not exist. Usage is: riel add -A/[file1 file2 file3...]",
                item
            );
            return false;
        }
